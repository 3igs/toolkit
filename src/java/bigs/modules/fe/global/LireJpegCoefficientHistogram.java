...
/**
 * LireJpegCoefficientHistogram.java
 * DescriptionClass
 * bigs
 ...
 * @since 		1.5
 * @history
 * 06/03/2012	LireJpegCoefficientHistogram.java
...
public class LireJpegCoefficientHistogram extends FeatureExtractionAlgorithm {
    ...
    private double[][][] dctScaler2 = new double[][][]
                                                     {
	...
	public List<List<Double>> extractFeatures(byte[] source) {			
        ...
        int values[] = (new int[BLOCK_SIZE * BLOCK_SIZE * 3]);
        
        getComponentHistogram(yuvImage, newWidth, newHeight, 0, values);
        getComponentHistogram(yuvImage, newWidth, newHeight, 1, values);
        getComponentHistogram(yuvImage, newWidth, newHeight, 2, values);
